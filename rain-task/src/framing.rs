...
impl SocketExt for UnixStream {
    ...
    fn read_msg(&mut self) -> Result<WorkerToSubworkerMessage> {
        ...
        let msg = serde_cbor::from_slice::<WorkerToSubworkerMessage>(&data).expect("error parsing message as CBOR");
        Ok(msg)
    }

    fn write_frame(&mut self, data: &[u8]) -> Result<()> {
        if data.len() > MAX_MSG_SIZE {
            panic!("write_frame: message too long ({} bytes of {} allowed)", data.len(), MAX_MSG_SIZE);
        }
    ...
    fn read_frame(&mut self) -> Result<Vec<u8>> {
        ...
        if len > MAX_MSG_SIZE {
            panic!("read_frame: message too long ({} bytes of {} allowed)", len, MAX_MSG_SIZE);
        }
