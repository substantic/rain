...
impl Subworker {
    ...
    pub fn run(&mut self) {
        ...
        if self.staging_dir.exists() || self.tasks_dir.exists() {
            panic!("Subworker must be ran in a clean directory (workdir: {:?})", self.working_dir);
        }
        ...
        env::set_current_dir(self.socket_path.parent().unwrap()).expect("error chdir to socket dir");
        let mut sock = UnixStream::connect(&self.socket_path.file_name().unwrap()).expect("error opening socket");;
    ...
    fn handle_call(&mut self, call_msg: CallMsg) -> ResultMsg {
        ...
        match self.tasks.get(&call_msg.method) { 
            None => {
                debug!("Task {:?} not found in subworker {:?}", call_msg.method, self.subworker_type);
                context.success = false;
                context.attributes.set("error",
                    format!("Task {:?} not found in subworker {:?}", call_msg.method, self.subworker_type)).unwrap();
            },
            Some(f) => {
                ...
                if let Err(ref e) = res {
                    ...
                    if !self.keep_failed_tasks {
                        ...
                        fs::remove_dir_all(task_dir).expect("error removing the task direcotry after failure");
                    }
                } else {
                    debug!("Method {:?} finished", call_msg.method);
                    // cleanup of the task working dir
                    fs::remove_dir_all(task_dir).expect("error removing the finished task direcotry");
                }
            },
        }
