...
pub struct Frame {
    ...
    pub timestamp: SystemTime, // Sample timestamp
    net_stat: HashMap<String, Vec<u64>> // Network IO
}
...
mod tests {
    ...
    fn test_cpu_uasge() {
        let mut monitor = Monitor::new();
        let cpu_usage = monitor.get_cpu_usage(&(monitor.get_cpu_time()), SystemTime::now());
        for u  in cpu_usage {
            assert!(u >= 0);
            assert!(u <= 100)
        }
